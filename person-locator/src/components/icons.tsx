...
export function CitizenIcon({ active = false, className, size = 30 }: { active?: boolean; className?: string; size?: number }) {
  return (
    <div className={`relative shrink-0 ${active ? 'text-primary-main' : ''} ${className ?? ''}`} style={{ width: size, height: size }}>
...
export function ProfilePlaceholder({ photoUrl, online = false }: { photoUrl?: string; online?: boolean }) {
  return (
    <div className="flex items-start justify-end p-1 relative shrink-0">
      <div className="relative shrink-0 w-photo h-photo">
        <div className="absolute border border-photo-frame inset-[2.5%] rounded overflow-hidden">
          {photoUrl ? (
            ...
            <div className="absolute inset-0 bg-grey-100 flex items-center justify-center">
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-text-disabled" aria-hidden="true">
        ...
        {online && (
          <div className="absolute left-10 top-0.5 w-2.5 h-2.5 rounded-full bg-success-main border-2 border-white" />
        )}
      </div>
    </div>
  );
